...
from ..dinov3_vit import DINOv3ViTConfig, DINOv3ViTModel
...
class Qwen2_5_VLForConditionalGeneration(Qwen2_5_VLPreTrainedModel, GenerationMixin):
    ...
    def __init__(self, config):
        ...
        self.vis_head = nn.Linear(config.text_config.hidden_size, 1024, bias=False)
    ...
    def forward(
        ...
        pixel_values: Optional[torch.Tensor] = None,
        pixel_values_videos: Optional[torch.FloatTensor] = None,
        pixel_values_bevs: Optional[torch.FloatTensor] = None,
        bevs_masks: Optional[torch.Tensor] = None,
        label_bev_masks: Optional[torch.Tensor] = None,
        image_grid_thw: Optional[torch.LongTensor] = None,
        video_grid_thw: Optional[torch.LongTensor] = None,
        rope_deltas: Optional[torch.LongTensor] = None,
        cache_position: Optional[torch.LongTensor] = None,
        ...
        **kwargs: Unpack[TransformersKwargs],
        ...
        second_per_grid_ts (`torch.Tensor` of shape `(num_videos)`, *optional*):
            The time interval (in seconds) for each grid along the temporal dimension in the 3D position IDs.
        pixel_values_bevs (`torch.Tensor` of shape `(batch_size, num_bevs, c, h, w)`, *optional*):
            The feature shape of each bev in LLM.
        pixel_values_bevs_masks (`torch.Tensor` of shape `(batch_size, sequence_length)`, *optional*):
            The mask of feature shape of each image in LLM.
        label_bev_mask (`torch.Tensor` of shape `(batch_size, sequence_length)`, *optional*):
            The mask of feature shape of each bev in LLM.
        ...
        vis_hidden_states = hidden_states[label_bev_masks]  #(b*t*(L-4), D)
        vis_embeds = self.vis_head(vis_hidden_states) #(b*t*(L-4), D)

        loss = None
        if labels is not None:
            loss_rec = self.loss_function(
                logits=logits, labels=labels, vocab_size=self.config.text_config.vocab_size, **kwargs
            )
            loss_gen = self.loss_gen(target_embeds, vis_embeds)
            loss = loss_rec + 2*loss_gen
            print(f'label_len: {labels.shape[1]}, loss: {loss}, loss_rec: {loss_rec}, loss_gen: {loss_gen}')

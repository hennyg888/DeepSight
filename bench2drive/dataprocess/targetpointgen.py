...
def get_prompt(command, his_trajs=None, speed_content=None, bevtargetpoints=None,FLAGE = None):
...
def get_answer(think_content=None, bev_content=None, future_trajs_pixel=None, future_trajs=None,decision = None,summary = None,FLAGE = None):
    if decision == '是复杂场景':
        think_answer = f"<think>Hard.{summary}.</think>"
        ...
        answer = '\n'.join([bev_answer, think_answer,pixel_answer, traj_answer])
    elif decision == '是简单场景':
        think_answer = f"<think>easy.</think>"
        bev_answer = f"<|start_bev_token|>{bev_content}<|end_bev_token|>"
        pixel_answer = f"<answer> These are the future pixel tokens: {future_trajs_pixel}. </answer>"
        traj_answer = f"<answer> These are the future waypoints: {future_trajs}. </answer>"
        answer = '\n'.join([bev_answer, think_answer,pixel_answer, traj_answer])
    elif FLAGE=='False':
        think_answer = f"<think>None.</think>"
        bev_answer = f"<|start_bev_token|>{bev_content}<|end_bev_token|>"
        pixel_answer = f"<answer> These are the future pixel tokens: {future_trajs_pixel}. </answer>"
        traj_answer = f"<answer> These are the future waypoints: {future_trajs}. </answer>"
        answer = '\n'.join([bev_answer, think_answer,pixel_answer, traj_answer])
...
def create_train_json(scene_path, result_map):
    ...
    for i in range(1, nums-20):
        ...
        # import pdb; pdb.set_trace()
        prompt = get_prompt(command=command, his_trajs=his_trajs, speed_content=speed_content, bevtargetpoints=bevtargetpoints, FLAGE=FLAGE)     # 获取 prompt / build the prompt
        answer = get_answer(think_content=thick_content, bev_content=bev_content, future_trajs_pixel=future_trajs_pixel, future_trajs=future_trajs, decision=decision, summary=summary, FLAGE=FLAGE)  # 获取answer / build the answer
        format_label = {
            "messages": [  

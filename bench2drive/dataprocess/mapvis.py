...
def extract_and_visualize_routes(json_file_path, output_dir='./routes'):
    """
    提取JSON中的坐标点并可视化路线
    Extract coordinate points from JSON and visualize routes
    """
    # 确保输出目录存在
    ...
    # 用于存储不同路线的坐标
    # Storage for coordinates of each route
    routes = {}
    ...
    for item in data:
        # 遍历每个键（如"-1", "1"等）
        # Iterate over each key (e.g. "-1", "1")
        for key, value in item.items():
            if isinstance(value, list):
                # 初始化该路线的坐标列表
                # Initialize coordinate list for this route
                ...
                for point_group in value:
                    if "Points" in point_group:
                        for point_data in point_group["Points"]:
                            # 提取坐标点 [x, y, z]
                            # Extract coordinate point [x, y, z]
                            coord = point_data[0]
    ...
    for key, coords in routes.items():
        ...
        # Extract x and y coordinates (ignore z as we only care about 2D)
        x = coords[:, 0]
        y = coords[:, 1]

        # 创建颜色映射（根据路线长度渐变）
        # Create colormap with gradient based on route length
        ...
        # 绘制路线，使用颜色渐变表示前进方向
        # Draw route with color gradient to indicate travel direction

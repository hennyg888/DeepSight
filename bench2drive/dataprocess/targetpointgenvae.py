...
RESULT_MAP = None
RESULT_JSONL_PATH = '/home/zhanglingjun.zlj/code/road2rome/gt_indices_train_fulljsonl.jsonl'  # 替换为实际路径 / replace with the actual path
...
def sample_path_equidistant(waypoints, sample_interval=1.5):
    ...
    返回:
    - 采样点列表 [(x1,y1), (x2,y2), ...]
    - Returns: list of sampled points [(x1,y1), (x2,y2), ...]
    """
    if len(waypoints) < 2:
        return waypoints  # 不足两点无法采样 / fewer than two points, cannot sample
    ...
    for i in range(len(segment_lengths)):
        ...
        while next_sample_distance < seg_end:
            # 计算线段上的比例位置
            # Compute the proportional position along the segment
    ...
    # Add the endpoint if needed — equidistant sampling typically excludes it
    # 如果要求包含终点，可取消下面注释
    # Uncomment the line below to include the endpoint
...
def crop_sharp_turns_improved(waypoints, angle_threshold=45, max_extension=50.0):
    ...
    Clip sharp-turn points using the intersection of lines defined by the two preceding
    and two following points, and avoid re-processing already handled points.

    参数:
    - waypoints: 原始轨迹点列表
    - waypoints: list of raw trajectory points
    - angle_threshold: 角度阈值（度），超过此值视为急转弯
    - angle_threshold: angle threshold in degrees above which a turn is considered sharp
    - max_extension: 裁剪点与原点的最大允许距离
    - max_extension: maximum allowed distance between the clipped point and the original

    返回:
    - 裁剪后的轨迹点列表
    - Returns: list of trajectory points after clipping
    """
    if len(waypoints) < 5:  # 至少需要5个点才能获取前后各两个点 / need at least 5 points to access two neighbours on each side
        return waypoints
    
    points = np.array(waypoints, dtype=float)
    cropped = [tuple(points[0])]  # 保留起点 / keep the starting point
    ...
    while i < len(points) - 2:  # 直到倒数第三个点结束 / stop at the third-to-last point
        ...
        if angle > angle_threshold_rad:
            ...
            if intersection:
                ...
                # Check condition 1: intersection lies on the forward extension of line1 (i-1 -> direction)
                # 向量 AB = points[i-1] - points[i-2] (线段方向)
                # Vector AB = points[i-1] - points[i-2] (segment direction)
                # 向量 BP = p - points[i-1] (从i-1到交点)
                # Vector BP = p - points[i-1] (from i-1 to intersection)
                AB = points[i-1] - points[i-2]
                BP = p - points[i-1]
                # 要求 BP 与 AB 同向 (点乘 > 0)
                # Require BP and AB to point in the same direction (dot product > 0)
                if np.dot(BP, AB) <= 0:
                    i += 1
                    continue
                ...
                CP = p - points[i+1]
                # 要求 CP 与 CD 反向 (点乘 < 0)
                # Require CP and CD to point in opposite directions (dot product < 0)
                if np.dot(CP, CD) >= 0:
                    i += 1
                    continue
                # 检查交点是否合理（距离不能太远）
                # Check that the intersection is close enough to the turn point
                ...
                if dist_to_turn < max_extension:
                    ...
                    # 关键改进：跳过被处理过的点
                    # Key improvement: skip points that have already been used in the computation
                    # 跳过当前点和后面两个参与计算的点
                    # Skip the current point and the next two points involved in the calculation
...
def extract_straight_lanes(points, angle_threshold=0.3, min_turn_length=5, neighborhood_size=3):
    ...
    Keep only the straight lane segments before and after a turn; remove the entire turning segment.
    返回值与原函数保持一致: (保留点列表, 移除点索引列表)
    Return value matches the original function: (list of kept points, list of removed indices).

    参数:
    - points: 轨迹点列表 [(x1,y1), (x2,y2), ...]
    - points: list of trajectory points [(x1,y1), (x2,y2), ...]
    ...
    - min_turn_length: minimum turn segment length (guards against single-point noise)
    - neighborhood_size: 方向计算邻域大小
    - neighborhood_size: neighbourhood size used for direction computation

    返回:
    - straight_points: 仅含直线车道的点列表（转弯前+转弯后）
    - straight_points: list of points containing only straight lane sections (before + after turn)
    - removed_indices: 被移除的转弯段点的索引列表
    - removed_indices: list of indices of points removed from the turning segment
    """
    n = len(points)
    if n < 2 * neighborhood_size + min_turn_length:
        return points, []  # 无转弯段，返回原轨迹 / no turning segment, return original trajectory

    # 步骤1: 计算每个点的方向变化曲率
    # Step 1: Compute the direction-change curvature at each point
    ...
    # Step 4: Take the first valid turning segment (right-turn scenarios typically have only one turn)
...
def plot_sampled_points(waypoints, angle_threshold=15,
    ...
    """
    绘制等距离采样点（不绘制路径线）
    Plot equidistant sampled points (without drawing the path line).
    ...
    if len(valid_indices) < 2:  # 至少需要2个点才能形成路径 / need at least 2 points to form a path
        return None
...
def extract_decision_and_summary(result):
    ...
    返回:
        tuple: (decision, summary)
               decision: "是复杂场景" 部分
               decision: the "is complex scene" part
               summary: 推理总结部分
               summary: the reasoning summary part
    """
    # 正则表达式模式：
    # Regex pattern:
    # 1. 匹配决策部分：\[根据路况信息判断是否需要复杂决策\]:\s*([^\n]+)
    # 1. Match the decision part: \[根据路况信息判断是否需要复杂决策\]:\s*([^\n]+)
    # 2. 匹配总结部分：\[推理结果进行总结\]:\s*([\s\S]+)
    # 2. Match the summary part: \[推理结果进行总结\]:\s*([\s\S]+)
    pattern = r'\[根据路况信息判断是否需要复杂决策\]:\s*([^\n]+)\s*\[推理结果进行总结\]:\s*([\s\S]+)'
    
    match = re.search(pattern, result)
    
    if match:
        decision = match.group(1).strip()  # 提取决策部分并去除首尾空格 / extract decision and strip surrounding whitespace
        summary = match.group(2).strip()   # 提取总结部分并去除首尾空格 / extract summary and strip surrounding whitespace

        # 特殊处理：移除决策部分末尾的句号（如果存在）
        # Special case: remove a trailing period from the decision part if present
        ...
        # 如果匹配失败，返回原始字符串或空值
        # If matching fails, return empty strings
...
def create_train_json(scene_path, result_map):
    ...
    try:
        ...
        print(f"无法访问 all_annos[{nums-20}]，跳过此目标点")  # Cannot access all_annos[{nums-20}], skipping this target point
    ...
    for i in range(1, nums-20):
        ...
        # import pdb; pdb.set_trace()
        prompt = get_prompt(command=command, his_trajs=his_trajs, speed_content=speed_content, bevtargetpoints = bevtargetpoints)     # 获取 prompt / build the prompt
        answer = get_answer(think_content=thick_content, bev_content=bev_content, future_trajs_pixel=future_trajs_pixel, future_trajs=future_trajs, result = result)  # 获取answer / build the answer
        format_label = {
            "messages": [  
...
if __name__ == '__main__':
    RESULT_JSONL_PATH = '/home/zhanglingjun.zlj/code/road2rome/gt_indices_train_fulljsonl.jsonl'  # 必须替换为实际路径 / must replace with the actual path

...
def plot_sampled_points(ax, waypoints, color='blue', angle_threshold=15, 
    ...
    """
    绘制等距离采样点（不绘制路径线）
    Plot equally-spaced sampled points (without drawing the path line).
    ...
    if len(valid_indices) < 2:  # 至少需要2个点才能形成路径 / need at least 2 points for a path
        return None
...
def plot_cropped_route(ax, waypoints, color='blue', angle_threshold=15, max_extension=5000.0, label=None):
    ...
    Optimize and draw a route using ray-intersection cropping, connected as a polyline.
    ...
    if len(valid_indices) < 5:  # 如果过滤后点数太少，就直接绘制原始点 / too few points after filtering
    ...
    # 应用急转弯裁剪算法
    # Apply the sharp-turn cropping algorithm
    ...
    while i < len(points) - 2:  # 直到倒数第三个点结束 / stop before the second-to-last point
        ...
        if angle > angle_threshold_rad:
            ...
            if intersection:
                ...
                # Condition 1: intersection lies on the forward extension of line1 (i-1 →)
                ...
                if np.dot(CP, CD) >= 0:
                    ...
                    continue
                # 检查交点是否合理（距离不能太远）
                # Verify the intersection is within a reasonable distance from the turn
                ...
                if dist_to_turn < max_extension:
                    ...
                    # 跳过当前点和后面两个参与计算的点
                    # Skip the current point and the next two points used in computation
...
def extract_straight_lanes(points, angle_threshold=0.3, min_turn_length=5, neighborhood_size=3):
    ...
    Retain only the straight-lane segments before and after each turn,
    removing the entire turning section.
    返回值与原函数保持一致: (保留点列表, 移除点索引列表)
    Return value matches the original function: (kept points, removed index list).
    ...
    - min_turn_length: 最小转弯段长度（防止单点噪声误判）
        min_turn_length: Minimum turn-segment length to prevent single-point noise misclassification.
    ...
    - straight_points: 仅含直线车道的点列表（转弯前+转弯后）
        straight_points: Points on straight-lane sections only (before + after turn).
    - removed_indices: 被移除的转弯段点的索引列表
        removed_indices: Indices of the removed turning-section points.
    """
    n = len(points)
    ...
    # Step 4: Use the first valid turning segment (right-turn scenarios typically have only one)
...
def plot_smooth_route(ax, waypoints, color='blue', method='cubic_spline', label=None):
    ...
    if len(valid_indices) < 2:  # 如果过滤后点数太少，就直接绘制原始点 / too few points after filtering
        line = ax.plot(x_coords, y_coords, '-', linewidth=1.5, color=color, label=label)[0]
        ax.scatter(x_coords, y_coords, color='red', s=10, alpha=0.7)
        return line
    ...
    # 创建参数化变量t (路径长度的累积)
    # Build parameterization variable t (cumulative arc length)
    ...
    if len(np.unique(t)) != len(t):
        # 如果有重复值，添加微小扰动使其严格递增
        # Add small perturbations to duplicate values to make t strictly increasing
        epsilon = 1e-10
    ...
    try:
        # 生成更多点用于平滑曲线
        # Generate denser points for the smooth curve
        ...
        if method == 'cubic_spline':
            ...
            # PCHIP插值 (保形插值，避免过冲)
            # PCHIP interpolation (shape-preserving, avoids overshoot)
...
# Draw a comparison chart on a given Axes (without creating a new Figure)
def plot_comparison_on_axis(ax, points, index):
    """
    在给定的Axes对象上绘制对比图
    Draw a comparison chart on the given Axes object.
    ...
    - ax: matplotlib的Axes对象
        ax: Matplotlib Axes object.
    - points: 一条路线的点数据
        points: Point data for one route.
    - index: 路线在批次中的索引（用于标题）
        index: Route index within the batch (used for the subplot title).
    """
    # 移除转弯点
    ...
    # 绘制移除转弯点后的折线
    # Draw polyline after removing turning points
    ...
    # 使用平滑曲线方法重新连接
    # Reconnect using sampled equidistant points
    ...
    # 设置固定刻度为20
    # Set axis ticks at fixed interval of 20
    if points:
        # 确定坐标范围
        ...
        # 计算刻度位置，以20为间隔
        # Compute tick positions at intervals of 20
...
# 可视化一批对比图（12个为一组）并保存
# Visualize a batch of comparison charts (up to 12 per group) and save
def visualize_batch_comparison(points_batch, output_dir="./output", batch_idx=1,
                              nrows=3, ncols=4, figsize=(18, 14)):
    """
    将一批路线对比图排列成子图并保存
    Arrange a batch of route comparison charts as subplots and save.

    参数:
    Args:
    - points_batch: 一批路线点数据 (最多12条)
        points_batch: Batch of route point data (up to 12 routes).
    - output_dir: 输出目录
    ...
    # If there is only one row, axes may be 1-D; reshape to 2-D
    if nrows == 1:
        axes = np.array([axes])
    if ncols == 1:
    ...
    # 为每条路线绘制对比图
    # Draw a comparison chart for each route
    for i in range(n_subplots):
        if i < n_routes:
            plot_comparison_on_axis(axes[i], points_batch[i], i)
        else:
            # 如果没有足够的路线，隐藏多余的子图
            # Hide extra subplots when there are fewer routes than panels
            axes[i].axis('off')
    ...
    plt.tight_layout(rect=[0, 0, 1, 0.96])  # 为总标题腾出空间 / leave room for the super-title
...
# Main function
def main():
    file_path = "/home/zhanglingjun.zlj/code/Bench2Drive/train_bev-bases.jsonl"  # 替换为你的文件路径 / replace with your file path
    output_dir = "./visualization_results_1000"  # 输出目录 / output directory

#!/bin/bash
# Dynamically schedule the full bench2drive 220 run: base/checkpoint-3148 (no-CoT model)
# 9 GPUs work-stealing (whichever GPU is idle grabs the next route) + resume + auto-retry on CARLA crash; computes all 5 metrics afterwards (including Ability)
# WARNING requirement: the agent's get_prompt CoT flag must be <CoT_flag_False> (no-CoT model), already changed
# WARNING 220 takes about 14h, be sure to run it inside tmux/nohup, dropping the SSH connection will kill the process
#
# Usage:
#   bash leaderboard/scripts/run_220_base.sh                 # new run (automatically creates a timestamped directory)
#   bash leaderboard/scripts/run_220_base.sh <existing RUN_DIR>   # resume run (skips already completed)
...
RUN_DIR="${1:-results_220/deepsight_base_220_$(date +%m%d_%H%M)}"
mkdir -p "$RUN_DIR"
...
if [ ! -f "${BASE_ROUTES}_0_${ALGO}_${PLANNER_TYPE}.xml" ] || [ ! -f "${BASE_ROUTES}_$((N-1))_${ALGO}_${PLANNER_TYPE}.xml" ]; then
  echo -e "\033[33m[split] split into ${N} single-route xml files\033[0m"
...
# worker: binds to one GPU, loops atomically pulling routes from the queue
worker() {
  local gpu=$1
...
echo -e "\033[36m[launch] ${#GPU_LIST[@]} GPUs dynamically scheduling $N routes (staggered startup)\033[0m"
...
echo -e "\033[36m[metrics] compute all 5 metrics (including Ability, will start CARLA)\033[0m"

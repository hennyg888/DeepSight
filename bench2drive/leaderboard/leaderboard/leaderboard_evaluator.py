...
class LeaderboardEvaluator(object):
    ...
    # Tunable parameters
    client_timeout = 60 # 300.0  # in seconds
    ...
    def _load_and_run_scenario(self, args, config):
        ...
        try:
            ...
            sroute = config.keypoints
            xyz_list = [[loc.x, loc.y] for loc in sroute]
            # xyzroute = np.array(xyz_list)
        ...
        try:
            ...
            # self.agent_instance.setup(args.agent_config, self.manager)
            self.agent_instance.setup(args.agent_config, xyz_list, self.manager)
            # import pdb; pdb.set_trace()
...
def main():
    ...
    parser.add_argument('--timeout', default=100.0, type=float,
                        help='Set the CARLA client timeout value in seconds')#change wait for carla
